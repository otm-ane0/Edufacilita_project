                ...
                <div class="group bg-gradient-to-br from-orange-50 to-red-100 rounded-2xl p-8 border border-orange-200 hover:border-orange-300 transition-all duration-300 hover:shadow-lg">
                    <div class="flex items-center space-x-3 mb-8">
                        ...
                        <div>
                            ...
                            <p class="text-gray-600 text-sm">Update LaTeX formatted question and answer (optional)</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="space-y-3">
                            ...
                            <textarea name="latex_question" id="latex_question" rows="5"
                                      ...
                                      placeholder="Enter LaTeX formatted question... e.g., \frac{1}{2} + \frac{1}{3} = ?">{{ old('latex_question', $question->latex_question) }}</textarea>
                        ...
                        <div class="space-y-3">
                            ...
                            <textarea name="latex_answer" id="latex_answer" rows="4"
                                      ...
                                      placeholder="Enter LaTeX formatted answer... e.g., \frac{5}{6}">{{ old('latex_answer', $question->latex_answer) }}</textarea>
                        ...
                        <div class="space-y-3">
                            <label class="flex items-center text-sm font-bold text-gray-800 uppercase tracking-wide">
                                ...
                                Current Images ({{ count($question->images) }})
                            </label>
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                <div class="bg-white border-2 border-gray-200 rounded-xl p-4 shadow-sm relative">
                                    <img src="{{ asset('storage/' . $imagePath) }}" alt="Question image {{ $index + 1 }}" 
                                         class="w-full h-32 object-cover rounded-lg shadow-md">
                                    <p class="text-sm text-gray-600 mt-2 text-center">{{ basename($imagePath) }}</p>
                        ...
                        <div class="space-y-3">
                            <label for="image_files" class="flex items-center text-sm font-bold text-gray-800 uppercase tracking-wide">
                                ...
                                {{ ($question->images && count($question->images) > 0) ? 'Add More Images' : 'Upload Question Images' }}
                            </label>
                            <div class="relative">
                            ...
                            <div class="flex items-center space-x-2 text-sm text-green-700 bg-green-50 p-3 rounded-lg">
                                ...
                                <span>{{ ($question->images && count($question->images) > 0) ? 'Upload additional images to add to the existing ones' : 'Upload multiple image files (JPG, PNG, GIF, WebP) for this question' }}. Hold Ctrl/Cmd to select multiple files.</span>
                            </div>
                        </div>
                    </div>
                </div>
...
>>>>>>> fdaa374b8c473690086850ea4e7af998f74c278c
    ...
    function initRichEditor(selector){
        ...
        if(!el) return;
        // Remove native required to avoid hidden focus error
        if(el.hasAttribute('required')) el.removeAttribute('required');
        ClassicEditor.create(el, {
            extraPlugins: [ FileUploadAdapterPlugin ],
            toolbar: {
                // Removed underline, subscript, superscript (not in default classic build)
        ...
        form.addEventListener('submit', function(e){
            ...
            for(const sel of requiredEditors){
                ...
                if(plain === ''){
                    ...
                    submitBtn.innerHTML = '<i class="fas fa-save group-hover:scale-110 transition-transform duration-300"></i><span>Update Question</span>';
