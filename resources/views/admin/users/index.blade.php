...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function storeUserData() {
        ...
        allUsers = Array.from(rows).map(row => {
            ...
            if (cells.length === 0) return null; // Skip empty state row
            
            return {
                element: row.cloneNode(true),
                id: row.getAttribute('data-user-id'),
                name: cells[0].textContent.trim().toLowerCase(),
                email: cells[1].querySelector('div:first-child').textContent.trim().toLowerCase(),
                phone: cells[1].querySelector('div:nth-child(2)') ? cells[1].querySelector('div:nth-child(2)').textContent.trim() : '',
                institution: cells[2].textContent.trim(),
                credit: parseFloat(cells[3].querySelector('.font-bold').textContent.replace(/[,$]/g, '')),
                role: cells[4].querySelector('span').textContent.trim().toLowerCase(),
                createdAt: new Date(cells[5].querySelector('div:first-child').textContent.trim()),
                rawData: {
                    name: cells[0].textContent.trim(),
                    email: cells[1].querySelector('div:first-child').textContent.trim(),
                    phone: cells[1].querySelector('div:nth-child(2)') ? cells[1].querySelector('div:nth-child(2)').textContent.trim() : '',
                    institution: cells[2].textContent.trim(),
                    credit: cells[3].querySelector('.font-bold').textContent.trim(),
                    role: cells[4].querySelector('span').textContent.trim(),
                    createdAt: cells[5].querySelector('div:first-child').textContent.trim()
                }
            };
    ...
    function sortUsers() {
        ...
        filteredUsers.sort((a, b) => {
            switch(sortCriteria) {
                ...
                case 'credit':
                    return b.credit - a.credit; // Highest first
                default:
                    return 0;
            }
        });
    ...
    function renderUsers() {
        ...
        if (filteredUsers.length === 0) {
            ...
            emptyRow.innerHTML = `
                <td colspan="7" class="px-6 py-16 text-center">
                    <div class="flex flex-col items-center animate-pulse">
                        ...
                        <h3 class="text-xl font-semibold text-gray-700 mb-2">No users found</h3>
                        <p class="text-gray-500 text-base mb-6 max-w-md">No users match your current search criteria. Try adjusting your filters or search term.</p>
                        <button onclick="resetFilters()" class="bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white px-6 py-3 rounded-lg font-semibold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 flex items-center space-x-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                            </svg>
            ...
            filteredUsers.forEach(user => {
                const newRow = user.element.cloneNode(true);
                // Re-attach event listeners for forms
                const deleteForm = newRow.querySelector('form');
                if (deleteForm) {
    ...
    function updateResultsCount() {
        const countElements = document.querySelectorAll('.text-sm.text-gray-500 span.font-medium, .text-sm.text-gray-600 span.font-bold');
        ...
        const showingElements = document.querySelectorAll('.text-sm.text-gray-700, .text-sm.text-gray-600');
        showingElements.forEach(element => {
            if (element.textContent.includes('Showing')) {
                const total = filteredUsers.length;
                element.innerHTML = `Showing <span class="font-bold text-blue-600">1</span> to <span class="font-bold text-blue-600">${total}</span> of <span class="font-bold text-blue-600">${total}</span> results`;
            }
        });
    }
